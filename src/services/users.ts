...
export class UsersService {
  ...
  public setRes(x: unknown): unknown {
    this.res = x;
    return this.res;
  }
  ...
  public async create(userCreationParams: UserCreationParams) {
    ...
    if (!emailRegexp.test(userCreationParams.email))
      console.log(
        this.setRes(new ApiError("Validation error", 400, "Email is invalid"))
      );
    ...
    await UserModel.findOne({ email: newUser.email }).then((data) => {
      if (data) {
        // This one doesn't work
        console.log(
          this.setRes(
            new ApiError("Not unique", 409, "Email is already in use")
          )
        );
      } else {
        newUser
